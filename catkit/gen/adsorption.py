...
class AdsorptionSites():
    ...
    def get_symmetric_sites(self, unique=True, screen=True):
        ...
        if symmetry_match is None:
            symmetry_match = symmetry_equivalent_points(
                self.frac_coords, self.slab)
...
def symmetry_equivalent_points(
    ...
    fractional_coordinates : ndarray (N, 3)
        ...
        between.
    atoms : Atoms object
        Atoms object to use the unit cell and positions.
    tol : float
    ...
    # Disallow cell inversion and z-translations
    nt = affine_matrices[:, 2, 3] < tol
    nt &= np.abs(affine_matrices[:, 2, 2] - 1) < tol
    affine_matrices = affine_matrices[nt]

    affine_points = np.insert(fractional_coordinates, 3, 1, 1)
